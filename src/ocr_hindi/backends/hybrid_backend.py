...
Pricing (per 1M tokens): $0.15 input / $0.60 output (standard)
Batch API: $0.075 input / $0.30 output (50% off)
Cost: ~$0.10-0.30 per 1000 pages (vs $2+ for pure Gemini)
"""
...
def _suppress_stdout():
    """Temporarily suppress stdout (for noisy library initialization)."""
...
class HybridBackend(OCRBackend):
    ...
    Uses gemini-2.0-flash with optimized settings:
    ...
    def __init__(
        ...
        self._primary: OCRBackend | None = None
    ...
    def cost_per_1000_pages(self) -> float:
        ...
        gemini_ratio = 0.15
        gemini = GeminiBackend(config=self.config, model=self.gemini_model)
        return gemini.cost_per_1000_pages * gemini_ratio
    ...
    def initialize(self) -> tuple[bool, str]:
        ...
        try:
            ...
            return False, f"Hybrid initialization failed: {exc}"
    ...
    def print_stats(self) -> None:
        ...
        stats_table.add_row(
            "  FREE (local)",
            str(stats["primary_only"]),
            f"[green]{primary_pct:.0f}%[/green]",
        )
        stats_table.add_row(
            "  Gemini API",
            str(stats["gemini_verified"]),
            f"[yellow]{gemini_pct:.0f}%[/yellow]",
        )
        stats_table.add_row("", "", "")

        stats_table.add_row("Tokens Used", f"{usage.total_tokens:,}", "")
        stats_table.add_row("  Input", f"{usage.input_tokens:,}", "$0.15/1M")
        stats_table.add_row("  Output", f"{usage.output_tokens:,}", "$0.60/1M")
        ...
        if stats["total_pages"] > 0:
            ...
            cost_text = Text()
            cost_text.append("Total: ", style="dim")
            cost_text.append(f"${usage.total_cost:.4f}", style="bold green")
            cost_text.append("  |  ", style="dim")
            cost_text.append("FREE pages: ", style="dim")
            cost_text.append(str(free_pages), style="bold yellow")
            cost_text.append("  |  ", style="dim")
            cost_text.append("API pages: ", style="dim")
            cost_text.append(str(api_pages), style="bold cyan")

            if free_pages > 0:
                savings_pct = (free_pages / stats["total_pages"]) * 100
                cost_text.append("  |  ", style="dim")
                cost_text.append("Savings: ", style="dim")
